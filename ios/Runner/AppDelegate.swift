...
@objc class AppDelegate: FlutterAppDelegate {
        ...
        if let controller = window?.rootViewController as? FlutterViewController {
            ...
            channel.setMethodCallHandler({ [weak self] (
                ...
                case "startLiveActivity":
                    guard let arguments = call.arguments as? [AnyHashable: String] else {
                        return
                    }
                    let durationString: String = arguments["duration"] as? String ?? ""
                    self?.startLiveActivity(duration: durationString)
    ...
    func startLiveActivity(duration: String) {
        ...
        let initialContentState = PizzaDeliveryAttributes.ContentState(driverName: duration, deliveryTimer:date)
